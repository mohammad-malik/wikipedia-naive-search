...
def load_tfidf_scores(file_path, vocabulary):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            scores = dict(
                (int(term_id), float(tfidf)) for term_id, tfidf in scores_list
            )
...
directory = 'C:\\Users\\Administrator\\Documents\\GitHub\\naive-search\\result\\'
...
def vectorize_query(query, vocabulary, idf_scores):
    ...
    for word in words:
        ...
        if term_id and term_id in idf_scores:
            index = list(vocabulary.keys()).index(term_id)
            vector[index] = idf_scores[
                term_id
            ]  # Using IDF score directly; adjust as needed
    return vector
...
def process_query(query, vocabulary, idf_scores, doc_vectors):
    ...
    similarity_scores = {
        doc_id: cosine_similarity(vector, query_vector)
...
def main():
    while True:
        ...
        if query.lower() == "exit":
            break
        ranked_documents = process_query(
            query,
            vocabulary,
            idf_scores,
            doc_vectors
        ...
        top_count = 5
        print(f"Top {top_count} documents ranked by similarity:")
        for doc_id, score in ranked_documents[:top_count]:
